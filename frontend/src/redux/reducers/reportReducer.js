import { accountReportURL, mainBookPageURL, reportAnalyticCardURL, reportBrutoBalance, reportBrutoBalanceURL } from "../api"
...
const REQUEST_SUCCESS_BB = createAction("REPORT/REQUEST_SUCCESS_BB")
...
export const generateBrutoBalanceReport = (payload) => async (dispatch) => {
	dispatch(START_REQUEST());
	return axios
		.post(reportBrutoBalanceURL, payload)
		.then((res) => handleResponse(res, dispatch, REQUEST_SUCCESS_BB, REQUEST_FAIL))
		.catch(error => console.log(error))
}
...
export const reportReducer = createReducer(initState, (builder) => {
	builder
		...
		.addCase(REQUEST_SUCCESS_BB, (state, action) => {
			state.loading = false
			state.bruto = action.payload
		})
